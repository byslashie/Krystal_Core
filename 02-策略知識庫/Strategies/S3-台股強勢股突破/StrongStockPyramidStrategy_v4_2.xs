{=====================================================================
  ...
  版本：v4.2 (2026/05/18)
  基於 v4.0-clean，加上「合理門檻」的贏家 trailing
  v4.0-clean → v4.2 變更：
    [新增] 贏家 trailing：浮盈 ≥ 100% 進贏家模式，從高點回吐 25% 出場
           設計理由：台股飆股 10 倍路上典型回檔 -15~-20%，回吐 25% 才視為真轉空
           不啟用鎖利 1/3（鎖利會直接砍金雞母肉，違反 Pareto 原則）
...
Inputs:
    ...
    // === v4.2 新增：贏家 trailing（合理門檻版） ===
    Enable_WinnerTrail(true, "是否啟用贏家 trailing"),
    Winner_ProfitThreshold(1.00, "贏家門檻浮盈 (100%，翻倍才算)"),
    Winner_TrailPct(0.25, "贏家 trailing 幅度 (25%，>典型回檔)");
...
Vars:
    ...
    IntraBarPersist WinnerModeOn(false),       // v4.2 贏家模式
    OrderQty(0),
...
If filled = 0 and IsNewMonth and isSessionFirstBar and isfirstcall("RealBar") then begin
    ...
    then begin
       ...
       PyramidCount = 0;
       WinnerModeOn = false;          // v4.2 重置贏家模式
...
if filled <> 0 then begin
    ...
    // A) 規則5：動態波動率防守停損（贏家模式不適用，避免被洗出）
    if Enable_Rule5 and WinnerModeOn = false
       and Close <= filledAvgPrice * (1 - (Vol20 * 4))
    then begin
        LastTradeReturn = ProfitPct;
        SetPosition(0, label:="動態Vol停損");
    end else

    // D.1) 規則6：時間上限強制出場 + 相對強弱特赦（贏家模式不適用）
