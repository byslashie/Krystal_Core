...
資料來源：
  ...
  - Schwab：broker_positions 分頁最新一批 Schwab 資料
...
daily_nav 欄位：
  date | yuanta_market_value | yuanta_unrealized_pnl | yuanta_positions_count
       ...
       | schwab_market_value | schwab_unrealized_pnl | schwab_positions_count
...
NAV_COLUMNS = [
    ...
    "ib_positions_count",
    "schwab_market_value",
    "schwab_unrealized_pnl",
    "schwab_positions_count",
    "total_market_value",
...
def _read_broker_from_sheets(sheet, broker_name: str) -> dict:
    """從 broker_positions 分頁讀取指定券商的最新持倉資料（IB 或 Schwab）"""
    try:
        ...
        for r in reversed(values[1:]):
            if len(r) > i_broker and r[i_broker] == broker_name:
        ...
        rows = [r for r in values[1:]
                if len(r) > i_broker
                and r[i_broker] == broker_name
                and (i_time is None or r[i_time] == last_ts)]

        mv  = sum(float(r[i_mv]  or 0) for r in rows if i_mv  is not None and len(r) > i_mv)
        pnl = sum(float(r[i_pnl] or 0) for r in rows if i_pnl is not None and len(r) > i_pnl)
        print(f"{broker_name} ({last_ts})：市值={mv:.0f}  未實現={pnl:.0f}  持倉={len(rows)} 檔")
        return {"market_value": mv, "unrealized_pnl": pnl, "count": len(rows)}
    except Exception as e:
        print(f"[WARN] 讀取 {broker_name} 資料失敗：{e}")
        return {}
...
def main() -> None:
    ...
    nav_sheet = get_sheet("daily_nav")
    if nav_sheet is None:
        print("[ERROR] 無法取得 daily_nav 分頁（請先在 Google Sheets 建立此分頁）")
        sys.exit(1)
    ...
    schwab = _read_broker_from_sheets(bp_sheet, "Schwab") if bp_sheet else {}
    ...
    s_mv  = schwab.get("market_value", 0)
    s_pnl = schwab.get("unrealized_pnl", 0)
    s_cnt = schwab.get("count", 0)
    ...
    row = [
        ...
        round(s_pnl, 2),
        s_cnt,
        round(y_mv + i_mv + s_mv,   2),
        round(y_pnl + i_pnl + s_pnl, 2),
    ]
    ...
    print(f"  Schwab 市值={s_mv:.0f}  未實現={s_pnl:.0f}  {s_cnt}檔")
    print(f"  合計   市值={y_mv+i_mv+s_mv:.0f}  未實現={y_pnl+i_pnl+s_pnl:.0f}")
    print("完成")
