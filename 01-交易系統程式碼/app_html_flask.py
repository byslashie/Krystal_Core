...
def get_broker_snapshot_from_sheets():
    """從 Google Sheets 讀取最新帳戶快照（聚合所有券商的最新數據）"""
    try:
        if not SHEETS_OK:
            return None
        ...
        if df is not None and not df.empty:
            ...
            broker_col = '券商' if '券商' in df.columns else 'broker'
            ...
            if time_col in df.columns:
                ...
                if broker_col in df.columns:
                    # 分群取得各券商的最新一筆
                    latest_by_broker = df.sort_values(time_col).groupby(broker_col).last()
                    
                    total_net_liquidation = 0.0
                    total_cash_value = 0.0
                    latest_timestamp = None
                    
                    for broker, row in latest_by_broker.iterrows():
                        asset_val = str(row.get(asset_col, '0')).replace(',', '')
                        cash_val = str(row.get(cash_col, '0')).replace(',', '')
                        
                        nlv = pd.to_numeric(asset_val, errors='coerce')
                        cash = pd.to_numeric(cash_val, errors='coerce')
                        
                        total_net_liquidation += 0.0 if pd.isna(nlv) else float(nlv)
                        total_cash_value += 0.0 if pd.isna(cash) else float(cash)
                        
                        ts = row.get(time_col, None)
                        if latest_timestamp is None or (ts is not pd.NaT and ts > latest_timestamp):
                            latest_timestamp = ts
    ...
    try:
        ...
        if snapshot and nav_df is not None:
            ...
            if positions:
                ...
                pos_list = list(deduped.values())
                holdings = len(pos_list)
                
                for pos in pos_list:
                    broker = pos.get('券商', pos.get('broker', '')).upper()
                    symbol = pos.get('symbol', '')
                    if 'IB' in broker or 'IBKR' in broker or (not broker and is_ib_symbol(symbol)):
                        ib_count += 1
                    elif 'YUANTA' in broker or '元大' in broker or (not broker and is_yuanta_symbol(symbol)):
                        yuanta_count += 1
                    elif 'SCHWAB' in broker or 'CHARLES SCHWAB' in broker:
                        schwab_count += 1
            ...
            return {
                ...
                'holdings': holdings,
                'ib_count': ib_count,
                'yuanta_count': yuanta_count,
                'schwab_count': schwab_count,
                'data_source': 'sheets'
            }
    except Exception as e:
